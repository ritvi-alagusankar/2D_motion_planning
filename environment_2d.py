...
class Environment(object):
  ...
  def random_query(self):
    ...
    for i in range(max_attempts):
      self.x_start = np.random.rand()*self.size_x
      self.y_start = np.random.rand()*self.size_y
      if not self.check_collision(self.x_start, self.y_start):
        found_start = True
        break
    for i in range(max_attempts):
      self.x_goal = np.random.rand()*self.size_x
      self.y_goal = np.random.rand()*self.size_y
      if not self.check_collision(self.x_goal, self.y_goal):
        found_goal = True
        break
    if found_start and found_goal:
      return self.x_start, self.y_start, self.x_goal, self.y_goal
    else:
      return None
